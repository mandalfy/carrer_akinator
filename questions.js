...
class ScoringEngine {
    ...
    reset() {
        ...
        for (const traitId in careerTraits) {
            ...
            for (const career in trait.careers) {
                if (!this.careerScores[career]) {
                    this.careerScores[career] = 0;
                }
            }
        }
    ...
    getNextQuestion() {
        // Find the trait that will best differentiate top careers
        ...
        // Select a good discriminating question
        // Prioritize traits that affect careers currently scoring high
        const topCareers = this.getTopCareers(5);
        let bestTrait = null;
        ...
        for (const traitId of remainingTraits) {
            ...
            // Score based on how many top careers this trait affects
            for (const career of topCareers) {
                if (trait.careers[career.id]) {
                    relevanceScore += trait.careers[career.id];
                }
            }
        ...
        // If no relevant trait found, pick randomly
        if (!bestTrait) {
    ...
    answerQuestion(answerId) {
        ...
        const weight = ANSWER_WEIGHTS[answerId];
        const trait = careerTraits[this.currentTraitId];

        // Update scores for all careers affected by this trait
        for (const career in trait.careers) {
            const traitWeight = trait.careers[career];
            this.careerScores[career] += weight * traitWeight;
        }
    ...
    getTopCareers(count = 3) {
        const sorted = Object.entries(this.careerScores)
            .sort((a, b) => b[1] - a[1])
            .slice(0, count)
    ...
    getConfidence() {
        const top = this.getTopCareers(2);
        if (top.length < 2) return 100;

        const gap = top[0].score - top[1].score;
        const maxPossibleGap = this.questionCount * 6; // max weight * max trait weight
        return Math.min(Math.round((gap / maxPossibleGap) * 100 + 50), 99);
    }
}
